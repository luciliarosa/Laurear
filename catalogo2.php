...
// Verificar a conexão
if (!$conexao) {
    echo '<script>window.alert("Não foi possível conectar ao banco");</script>';
}
...
// Verificar se o ID do produto foi passado pela URL para adicionar ao carrinho
...
$nome_usuario = isset($_SESSION['nome_usuario']) ? $_SESSION['nome_usuario'] : "Nome do Usuário";

// Obter a quantidade de itens no carrinho
$totalItensCarrinho = isset($_SESSION['carrinho']) ? count($_SESSION['carrinho']) : 0;
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body>
    ...
    <header>
        ...
        <div class="compras">
            <i class="bi bi-cart3"></i> <span id="cart"><?php echo $totalItensCarrinho; ?> itens no carrinho</span>
        </div>
    </header>
    ...
    <script>
        ...
        function addToCart(productId) {
            ...
            // Definir o método e o arquivo PHP a serem usados na solicitação AJAX
            xhr.open('GET', 'catalogo2.php?id_produto=' + productId, true);
            xhr.send();
        }
    </script>
</body>
